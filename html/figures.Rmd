...
for (eachclone in SA906a_allclones){
  figlab <- ggdraw() + draw_label(paste0('n=',SA906a_clonelabels$Freq[which(SA906a_clonelabels$Var1 == eachclone)]), size=12, x = 0.95, angle = 90, vjust = -0.1, hjust = 0) + theme(plot.margin = margin(0, 0, 0, 0))
  fig <- plotCNprofile(CNbins = SA906a_CNbins[which(SA906a_CNbins$cell_id == eachclone),], legend.position = "none", pointsize = 0.6) +
...
for (eachclone in OV081_allclones){
  figlab <- ggdraw() + draw_label(paste0('n=',OV081_clonelabels$Freq[which(OV081_clonelabels$Var1 == eachclone)]), size=12, x = 0.95, angle = 90, vjust = -0.1, hjust = 0) + theme(plot.margin = margin(0, 0, 0, 0))
  fig <- plotCNprofile(CNbins = OV081_CNbins[which(OV081_CNbins$cell_id == eachclone),], legend.position = "none", pointsize = 0.6) +
    theme(axis.text = element_text(size=10), axis.title = element_blank())
  assign(paste0('fig',eachclone), plot_grid(fig, figlab, rel_widths = c(1,0.03)))
  clonefigslist = append(clonefigslist,paste0('fig',eachclone))
}

fig3f <- plot_grid(plotlist=mget(rev(clonefigslist)), nrow = length(OV081_allclones))
fig3f
...
filtresultfile = filtresultfile[which(filtresultfile$treealign_id %in% levels(Idents(Ovarian.cancer))),]
...
fig4f <- ggplot(OV081_scRNA, aes(x=UMAP_1,y=UMAP_2,color=MT.ND3)) + geom_point(size=0.5) +
  scale_color_gradientn(colours = c("blue", "blue", "white", "red"), values = rescale(x = c(0, 2.5, 3.8, 5)), name = 'Expression') +
  ggtitle('MT-ND3') + scale_x_continuous(limits= c(-6,-2)) + scale_y_continuous(limits= c(1,4)) + theme_minimal() +
  theme(axis.text = element_blank(),
        axis.title = element_blank(),
        panel.grid = element_blank(),
        plot.title = element_text(size = 14),
...
# Import varmatrix file from Zenodo (https://zenodo.org/records/6998936)
...
# Add the sample_id, library_id, and clone information
filtvarmatrix$sample_id = filtvarmatrix$library_id
filtvarmatrix$library_id = apply(data.frame(filtvarmatrix$cell_id), 1, function(x) strsplit(x,'[.]')[[1]][2])
filtvarmatrix$clone_UMAP = resultfile$clone_UMAP[match(gsub('[.]','-',filtvarmatrix$cell_id),resultfile$cell_id)]
